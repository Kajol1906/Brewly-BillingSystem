...
export default function EventCalendar({ currentDate, events, onDateClick, selectedDate }: EventCalendarProps) {
  ...
  const getEventsForDate = (date: Date) => {
    return events.filter(e => {
      const eventDate = e.date instanceof Date ? e.date : new Date(e.date + 'T00:00:00');
            ...
            >
              ...
              {dayEvents.length > 0 && (
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block z-10">
                  <div className="bg-card border border-border rounded-lg p-2 shadow-lg min-w-[150px]">
                    {dayEvents.map(event => (
